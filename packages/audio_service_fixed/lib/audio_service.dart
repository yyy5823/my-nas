...
class AudioService {
  ...
  /// 强制刷新 iOS Now Playing 信息（绕过去重机制）
  ///
  /// iOS MediaRemote 框架有系统级去重机制，当 nowPlayingInfo 相同时会跳过更新。
  /// 这会导致 app 第二次进入后台时灵动岛不显示的问题。
  ///
  /// 此方法通过 "清空 → 延迟 → 重设" 策略强制 iOS 重新读取 nowPlayingInfo。
  ///
  /// 使用场景：
  /// - App 从后台返回前台再次进入后台时
  /// - 需要强制刷新灵动岛/锁屏显示时
  static Future<void> forceUpdateNowPlayingInfo() async {
    if (!Platform.isIOS && !Platform.isMacOS) return;

    const channel = MethodChannel('com.ryanheise.audio_service.client.methods');
    try {
      await channel.invokeMethod('forceUpdateNowPlayingInfo');
    } on PlatformException catch (e) {
      // 静默处理错误，避免影响正常流程
      debugPrint('AudioService: forceUpdateNowPlayingInfo failed: $e');
    }
  }
