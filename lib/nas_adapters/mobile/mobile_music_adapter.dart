...
/// 移动端音乐库适配器
///
/// 通过系统 API 访问手机音乐库和下载的音乐
/// 仅在 iOS/Android 平台可用
class MobileMusicAdapter implements NasAdapter {
  MobileMusicAdapter() {
    logger.i('MobileMusicAdapter: 初始化适配器');
  }

  late MobileMusicFileSystem _fileSystem;
  ...
  NasAdapterInfo get info => NasAdapterInfo(
        type: NasAdapterType.mobileMusic,
        name: '手机音乐',
  ...
  Future<ConnectionResult> connect(ConnectionConfig config) async {
    logger.i('MobileMusicAdapter: 开始连接');

    // 检查平台
    if (!Platform.isIOS && !Platform.isAndroid) {
      return const ConnectionFailure(
        error: '手机音乐仅支持 iOS 和 Android 平台',
      );
    }

    _config = config;
    _fileSystem = MobileMusicFileSystem();

    // 请求权限
    final hasPermission = await _fileSystem.requestPermission();
    if (!hasPermission) {
      return const ConnectionFailure(
        error: '未获得访问音乐库的权限，请在系统设置中授权',
      );
    }
    ...
    logger.i('MobileMusicAdapter: 连接成功 - ${_serverInfo!.hostname}');

    return ConnectionSuccess(
      sessionId: 'mobile_music_${DateTime.now().millisecondsSinceEpoch}',
      serverInfo: _serverInfo,
    );
  }
  ...
  Future<void> disconnect() async {
    ...
    _config = null;
    _serverInfo = null;
    logger.i('MobileMusicAdapter: 已断开连接');
  }
  ...
  Future<bool> checkConnectionHealth() async {
    // 移动端媒体始终可用（只要有权限）
    if (!_connected) {
      logger.d('MobileMusicAdapter: 连接健康检查 - 未连接');
      return false;
    }
    logger.d('MobileMusicAdapter: 连接健康检查 - 正常');
    return true;
  }
